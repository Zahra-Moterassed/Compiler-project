...
class DeclCheck : public ASTVisitor {
  ...
  */
  llvm::StringSet<> Scope; 	// -> a set to store the names
  bool HasError; 			// -> used to indicate that an error occurred
...
public:
  ...
  // RULE-1 : In a 'Factor' node that holds a variable name, we check that the variable name is in the set
  ...
  virtual void visit(WithDecl &Node) override { 
    for (auto I = Node.begin(), E = Node.end(); I != E; /*will be continued until we reach the end of the expression*/
         ++I) { 

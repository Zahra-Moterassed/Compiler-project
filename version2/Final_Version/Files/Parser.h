...
class Parser {
  ...
  Lexer &Lex; 	
  // instance of "Lexer" class ("Lex" -> retrieve the next token from the input)
  Token Tok;	
  // instance of "Token" class ("Tok" -> stores the next token (the look-ahead))
  bool HasError; 
  // "HasError" -> a flag that indicates if an error was detected

  void error() {
    llvm::errs() << "Unexpected: " << Tok.getText() << "\n";
    HasError = true;
  }
  // "advance()" -> retrieves the next token from lexer...
  // (use the "next" method of Lexer class)
  void advance() { Lex.next(Tok); }
  
  // "expect()" -> tests if the look-ahead is of the expected kind...
